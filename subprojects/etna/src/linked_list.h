...
#ifndef container_of
#define etna_container_of(ptr, type, member) \
    ((type *)((char *)(ptr) - offsetof(type, member)))
#else
#define etna_etna_container_of container_of
#endif

#define ETNA_LISTNODE_INIT (etna_listnode_t) { .prev = NULL, .next = NULL }

static inline void __list_add(etna_listnode_t *new_node,
                              etna_listnode_t *prev,
                              etna_listnode_t *next) {
    next->prev = new_node;
    new_node->next = next;
